...
| # | Thao tác | Mong đợi |
|---|---|---|
| A1 | Vào `/admin/courses` | Danh sách コース dạng thẻ: thumbnail + creator badge (管理者作成/教師作成) + tên + 作成者 + 作成日 + số 動画 + badge 公開/非公開 |
| A2 | Tab **作成者**: すべて / 管理者作成 N / 教師作成 N | Đổi tab → lọc đúng + số đếm khớp |
| A3 | Ô **キーワード** gõ tên khóa / nội dung / tên 作成者 | Lọc đúng cả 3 trường |
...
| B4 | Chọn file **không phải ảnh** | Báo lỗi định dạng |
| B5 | Đủ tên + ảnh → 保存 | Toast `コースを作成しました`; khóa mới xuất hiện đầu/list, status **非公開** mặc định |
...
## D. 動画 (SC-A10 upload + reorder + preview + delete)
| # | Thao tác | Mong đợi |
|---|---|---|
| D1 | **レッスン追加** → để trống レッスン名 hoặc chưa chọn file | Nút アップロード disabled |
| D2 | Chọn file video MP4 | Hiện tên file + dung lượng + **thời lượng (mm:ss)** tự đọc |
| D3 | Chọn file không phải video | Báo `MP4 / MOV / WebM ...` |
| D4 | Nhập レッスン名 + アップロード | Toast `「…」をアップロードしました`; video xuất hiện trong 動画一覧 |
| D5 | **Kéo icon ⠿ ở đầu dòng** đổi thứ tự | Số thứ tự cập nhật + toast `動画の順番を変更しました`; **reload vẫn giữ thứ tự** |
| D6 | Bấm **mắt 👁 / dòng video** | Modal xem trước, **video phát được** (player), hiện tên + 詳細 + thời lượng |
| D7 | Bấm **thùng rác** ở 1 video | Modal xác nhận → xoá → list cập nhật, toast |
